...
$(window).on('load.bs.select.data-api', function () {
    $.when(loadingFiisData("esp")).done(function () {
        estimatePayDateCalculate()
        ...
        $('#WithHoldingAmountInput').trigger('change')
        ...
        if ($('#ExceptedPaymentDateInput').val() == "" || $('#ExceptedPaymentDateInput').val() == "") {
        ...
        if ($('#WithHoldingAccountingAmountCode').val() != undefined && $('#WithHoldingAccountingAmountCode').val() != '') {
            $('#WithHoldingAccountingAmountDisable').text($('#WithHoldingAccountingAmountCode').val() + ' - ' + CodeGetOptionText('espAccountingItem', $('#WithHoldingAccountingAmountCode').val()))
        }
        //==========================================
        // 針對供應商載入設定
        //==========================================
        if ($('#VendorNumInput').val() != "" && $('#VendorNumInput').val() != undefined) {
            ...
            if ($('#PaymentMethodSelect').val() == 'WIRE') {
                if ($('#VendorKind').val() != "ONE TIME" && $('#VendorKind').val() != "EMPLOYEE_ESUN") {
                    generalWire()
        ...
        ////==========================================
        var espDetail = _espDetail;
        if (espDetail.length > 0) {
            $('#ESPDetailTable tbody').remove();
            $.each(espDetail, function (index, item) {
                ESPDetailDataAppend(item);
            });
        ...
        //==========================================
        $(document).on({
            change: MoneyHasBeenChanged
        }, '.moneyfield')
        $(document).on({
            change: CertificateAndTax
        }, '.taxfield')
        $('#CertificateAmountInput').bind('focus', RecordPreviousVal).bind('change', MoneyChange);
        ...
        $(document).on('change', 'select.AmortizationDetailAccountBankSelect', function () {
            ...
            if (typeof (Array.prototype.find) == "function") {
                ...
                $('#AmortizationDetailTable').find(detailIndex).val(_coaCompany.find(function (element) { return element.company == selectAccount }).description)
            }
        })
        ...
        // 憑證開立對象更改，判斷營業稅申報層
        //==========================================
        $(document).on('change', '#VoucherBeauSelect', judgeBusinessTaxGen);
        ...
        $(document).on('change', 'select.espExpenseAttributeSelect', judgeInvoiceDeductible);
        ...
        // 每次改動明細所扣欄位重新計算所扣金額
        //==========================================
        $(document).on('change', '#ESPDetailTable tbody #espIsIncomeDeductionSelect', IncomeTaxCal);
        $(document).on('change', '#ESPDetailTable tbody #espOriginalAmountInput', IncomeTaxCal);
        $(document).on('change', '#ESPDetailTable tbody #espOriginalTaxInput', IncomeTaxCal);
...
$(function () {
    ...
    //====================================================================================
    // 請款類別設定/修改所扣欄位/修改購買國外勞務欄位/修改統編欄位/修改憑證號碼/修改付款預扣金額(如：保固金)欄位/修改主旨欄位
    //====================================================================================
    ...
    $(document).on('change', '#Deduction', function () {
        if ($('#ExpenseKindSelect').val() == "ESP_CM_EXP") {
            $('#ESPDetailTable .espIsIncomeDeductionDisable').text("N");
            $('#ESPDetailTable .espIsIncomeDeductionSelect').val("N");
            return;
        }
        ...
        if ($(this).prop('checked')) {
            $('.espIsIncomeDeductionDisable').text("Y")
            $('.espIsIncomeDeductionSelect').val("true");
            $('#ESPDetailTable .espIsIncomeDeductionSelect').selectpicker('refresh');
            SwitchHideDisplay([$('#ESPDetailTable .espIsIncomeDeductionDisable')], [$('#ESPDetailTable .espIsIncomeDeductionSelect')])
            ...
            if ($('#VendorKind').val() == "ONE TIME") {
                ...
                $IncomeTable.find('select#CertficateKindSelect').selectpicker();
                if ($('#identityFlag').val() == "" || $('#identityFlag').val() == undefined) {
                    $IncomeTable.find('select#CertficateKindSelect').val("");
                ...
                SwitchHideDisplay(
                    [$IncomeTable.find('#IsTwoHealthInsuranceDisable'), $IncomeTable.find('#LeaseTaxDisable'), $IncomeTable.find('#LeaseAddressDisable'), $IncomeTable.find('div#CertficateKindDisable')]
                    , [$IncomeTable.find('#IsTwoHealthInsuranceSelect'), $IncomeTable.find('#LeaseTaxInput'), $IncomeTable.find('#LeaseAddressInput'), $IncomeTable.find('select#CertficateKindSelect')]
                )
            ...
            $('.espIsIncomeDeductionDisable').text("N")
            $('.espIsIncomeDeductionSelect').val("false");
            $('#ESPDetailTable .espIsIncomeDeductionSelect').selectpicker('refresh');
            SwitchHideDisplay([$('#ESPDetailTable .espIsIncomeDeductionSelect')], [$('#ESPDetailTable .espIsIncomeDeductionDisable')])

            SwitchHideDisplay([$('#IncomeTaxTable')], [$('#undone-GenerateIncomeTax')])
        }
        var $incomeTaxDetail = $('#IncomeTaxTable #accounting-stage-field');
        ...
        SwitchHideDisplay([$('#LeaseTaxCodeSelect'), $('#TwoHeathInsuranceFlagInput')], [$('#LeaseTaxCodeDisable'), $('#TwoHeathInsuranceFlagDisable')])
    ...
    $(document).on('change', '#WithHoldingAmountInput', function () {
        var value = parseInt($(this).val());
        if (value != 0 && !isNaN(value)) {
            $('#WithHoldingAccountingAmount').show();
            $('#WithHoldingAccountingAmountSelectBtn').show();
        } else {
            $('#WithHoldingAccountingAmount').hide();
            $('#WithHoldingAccountingAmountSelectBtn').hide();
            $('#WithHoldingAccountingAmountCode').val("");
            $('#WithHoldingAccountingAmountDisable').text("");
        }
    })
    $(document).on('change', '#HeaderDescInput', function () {
        $('[id="espVoucherMemoInput"]').val($(this).val())
    })
    ...
    $('#VoucherBeauSelect').on('change', function () {
        ...
        if (_firstVoucherBeau) {
            $('#AmortizationDetailTable').find('.AmortizationDetailAccountBankSelect').val($voucher.attr('accountCode'))
            try {
                if (typeof (Array.prototype.find) == "function") {
    ...
    //==========================================
    $(document).on('click', '#ExpandESPDetail', function () {
        if ($(this).find('.glyphicon-chevron-down').length > 0) {
            SwitchHideDisplay([], [$(this).parents('tbody').find('.ESPDetail')]);
        } else {
            SwitchHideDisplay([$(this).parents('tbody').find('.ESPDetail')], []);
        }
        $(this).find('.toggleArrow').toggleClass('glyphicon-chevron-down glyphicon-chevron-up')
    });
    $(document).on('click', '#ExpandAllESPDetail', function () {
        if ($(this).find('.list-open-icon').length > 0) {
            SwitchHideDisplay([], [$('.ESPDetail')]);
    ...
    $(document).on('click', '#AppendESPDetail', function () {
        ESPDetailAppend();
    });
    $(document).on('click', '#DeleteThisESPDetail', function () {
        ...
        if (removeTarget.find('#LineNum').val() != 0) {
            $('#InformationZone').append('<input type="hidden" name="ESPDetail.Index" value="' + removeTarget.find('#LineNum').val() + '">')
            $('#InformationZone').append('<input type="text" hidden name="ESPDetail[' + removeTarget.find('#LineNum').val() + '].LineNum" value="' + removeTarget.find('#LineNum').val() + '">')
            $('#InformationZone').append('<input type="text" hidden name="ESPDetail[' + removeTarget.find('#LineNum').val() + '].IsDelete" value="true">')
        }
        $(this).parents('tbody').remove();
        RenumberingDetail($('table#ESPDetailTable tbody'), 'DetailSerno')
        deletedRecalculateMoney($(this).parents('tbody'))
    });
    $(document).on('click', 'button[data-id="espPaymentCategorySelect"]', function () {
        var target = $(this).siblings('#espPaymentCategorySelect')
        if (target.find('option').length < 2) {
            LoadingPaymentCategory(target)
        }
    })
    $(document).on('change', "select.espPaymentCategorySelect", function () {
        var $value = $(this).val();
        var $midCategory = $(this).parents('tr').find('select.espPaymentMidCategorySelect');
        LoadingPaymentMidCategory($midCategory, $value)
    })
    $(document).on('change', "select.espPaymentMidCategorySelect", function () {
        var $catValue = $(this).parents('tr').find('select.espPaymentCategorySelect').val()
        var $midValue = $(this).val()
        var $expense = $(this).parents('tr').find('select.espExpenseAttributeSelect');
        LoadingExpenseAttribute($expense, $catValue, $midValue)
    })
    $(document).on('change', "select.espExpenseAttributeSelect", function () {
        var $catValue = $(this).parents('tr').find('select.espPaymentCategorySelect').val()
        var $midValue = $(this).parents('tr').find('select.espPaymentMidCategorySelect').val()
        var $expenseValue = $(this).val();
        LoadingAccountingItem($(this), $catValue, $midValue, $expenseValue)
    })
    $(document).on('click', 'button[data-id="espProjectCategorySelect"]', function () {
        var target = $(this).siblings('#espProjectCategorySelect')
        if (target.find('option').length < 2) {
            LoadingProjectCategory(target)
        }
    })
    $(document).on('change', "select.espProjectCategorySelect", function () {
        var $value = $(this).val();
        var $Project = $(this).parents('tr').find('select.espProjectSelect');
        LoadingProject($Project, $value)
    });
    $(document).on('change', "select.espProjectSelect", function () {
        var $value = $(this).val();
        var $ProjectItem = $(this).parents('tr').find('select.espProjectItemSelect');
...
// 預計付款日期計算邏輯
//==============================================================================================
function estimatePayDateCalculate() {
    if ($('#P_JBPMUID').val() == undefined || $('#P_JBPMUID').val() == "") {
        var Today = new Date();
        if ($('#VendorKind').val() != "EMPLOYEE_ESUN") {
            if (Today.getDate() >= 1 && Today.getDate() <= 6) {
                $('#EstimatePayDateDisable').text(Today.getFullYear() + '年' + (Today.getMonth() + 1) + '月' + '14日');
                $('input[name="EstimatePayDate"]').val(Today.getFullYear() + '-' + (Today.getMonth() + 1) + '-' + '14')
            } else if (Today.getDate() >= 7 && Today.getDate() <= 20) {
                $('#EstimatePayDateDisable').text(Today.getFullYear() + '年' + (Today.getMonth() + 1) + '月' + '28日');
                $('input[name="EstimatePayDate"]').val(Today.getFullYear() + '-' + (Today.getMonth() + 1) + '-' + '28')
            } else {
                if (Today.getMonth == 11) {
                    $('#EstimatePayDateDisable').text((Today.getFullYear() + 1) + '年' + 1 + '月' + '14日');
                    $('input[name="EstimatePayDate"]').val((Today.getFullYear() + 1) + '-' + (1) + '-' + '14')
                } else {
                    $('#EstimatePayDateDisable').text(Today.getFullYear() + '年' + (Today.getMonth() + 2) + '月' + '14日');
                    $('input[name="EstimatePayDate"]').val(Today.getFullYear() + '-' + (Today.getMonth() + 2) + '-' + '14')
                }
            }
        } else {
            if (Today.getDate() >= 1 && Today.getDate() <= 5) {
                $('#EstimatePayDateDisable').text(Today.getFullYear() + '年' + (Today.getMonth() + 1) + '月' + '16日');
                $('input[name="EstimatePayDate"]').val(Today.getFullYear() + '-' + (Today.getMonth() + 1) + '-' + '16')
            } else if (Today.getDate() >= 6 && Today.getDate() <= 16) {
                $('#EstimatePayDateDisable').text(Today.getFullYear() + '年' + (Today.getMonth() + 1) + '月' + '27日');
                $('input[name="EstimatePayDate"]').val(Today.getFullYear() + '-' + (Today.getMonth() + 1) + '-' + '27')
            } else if (Today.getDate() >= 17 && Today.getDate() <= 25) {
                if (Today.getMonth == 11) {
                    $('#EstimatePayDateDisable').text((Today.getFullYear() + 1) + '年' + 1 + '月' + '06日');
                    $('input[name="EstimatePayDate"]').val((Today.getFullYear() + 1) + '-' + (1) + '-' + '06')
                } else {
                    $('#EstimatePayDateDisable').text(Today.getFullYear() + '年' + (Today.getMonth() + 2) + '月' + '06日');
                    $('input[name="EstimatePayDate"]').val(Today.getFullYear() + '-' + (Today.getMonth() + 2) + '-' + '06')
                }
            } else {
                if (Today.getMonth == 11) {
                    $('#EstimatePayDateDisable').text((Today.getFullYear() + 1) + '年' + 1 + '月' + '16日');
                    $('input[name="EstimatePayDate"]').val((Today.getFullYear() + 1) + '-' + (1) + '-' + '16')
                } else {
                    $('#EstimatePayDateDisable').text(Today.getFullYear() + '年' + (Today.getMonth() + 2) + '月' + '16日');
                    $('input[name="EstimatePayDate"]').val(Today.getFullYear() + '-' + (Today.getMonth() + 2) + '-' + '16')
                }
            }
        }
    }
...
function vendorSelected(vendor) {
    ...
    if ($('#Deduction').prop('checked')) {
        $('#TwoHeathInsuranceFlagDisable').text($('#twoNhiFlag').val())
        $('#TwoHeathInsuranceFlagInput').val($('#twoNhiFlag').val())
    }
    $('#BusinessNumInput').val(vendor.vatRegistrationNumber)
    $('#BusinessNumDisable').text(vendor.vatRegistrationNumber);
...
function PleasePaymentSet() {
    ...
    $('#CertificateKindSelect').append(
        $('<option>', { value: 'I', text: '發票' }),
        $('<option>', { value: 'R', text: '收據' }),
        $('<option>', { value: 'O', text: '其他' })
    );
...
function DiscountSet() {
    ...
    $('#CertificateKindSelect option').remove();
    $('#CertificateKindSelect').selectpicker({ title: '請選擇' }).selectpicker('render');
    $('#CertificateKindSelect').append($('<option>', { value: 'C', text: '折讓' }));
...
function DonateSet() {
    ...
    $('#CertificateKindSelect option').remove();
    $('#CertificateKindSelect').selectpicker({ title: '請選擇' }).selectpicker('render');
    $('#CertificateKindSelect').append($('<option>', { value: 'I', text: '發票' }), $('<option>', { value: 'R', text: '收據' }), $('<option>', { value: 'O', text: '其他' }));
...
function initPleaseTypeSet() {
    ...
    if ($('#ExpenseKindSelect').val() != "" && $('#ExpenseKindSelect').val() != undefined) {
        SwitchHideDisplay([], [$('#ESPInfomationBlock'), $('#VoucherInfomationBlock')]);
    }
...
//==============================================================================================
// 請款明細層 - 建立/新增(有無資料)/請款大類/請款中類/費用屬性/專案類別/專案/專案項目/重新計算金額/刪除品項重新計算金額
//==============================================================================================
function CreateESPDetail() {
    if ($('#ESPDetailTable').length < 1) {
        var template = $('#ESPDetailTableTemplate').clone().attr('id', 'ESPDetailTable');
        $(template).find('tbody').remove();
        $('#ESPDetailRoot').append($(template).show())
        ESPDetailAppend();
    }
}
function ESPDetailAppend() {
    var count = $('#ESPDetailTable tbody').length;
    var $template = $('#ESPDetailCloneTemplate').clone().attr('id', 'ESPDetail_' + count);

    DisableDOMObject($template.find('select.espPaymentMidCategorySelect'));
    DisableDOMObject($template.find('select.espExpenseAttributeSelect'));
    DisableDOMObject($template.find('select.espProjectSelect'));
    DisableDOMObject($template.find('select.espProjectItemSelect'));

    $template.find('td.DetailSerno').text(count + 1);
    $template.find('.espAmortizationStartDateInput').datetimepicker({
        format: 'YYYY-MM-DD'
    });
    $template.find('.espAmortizationEndDateInput').datetimepicker({
        format: 'YYYY-MM-DD'
    });

    $template.find('#espOriginalAmountInput').bind('focus', RecordPreviousVal).bind('change', MoneyChange).bind('change', recalculateMoney);
    $template.find('#espOriginalTaxInput').bind('focus', RecordPreviousVal).bind('change', MoneyChange).bind('change', recalculateMoney);
    $('#ESPDetailTable').append($template);

    TableBindingSetting($('#ESPDetailTable'), 'ESPDetail');
    $template.find('.selectpickerInDetail').selectpicker();
    if ($('#Deduction').prop('checked')) {
        $template.find('select.espIsIncomeDeductionSelect').val("true").selectpicker('refresh')
        $template.find('div.espIsIncomeDeductionSelect').show()
        $template.find('#espIsIncomeDeductionDisable').hide()
    } else {
        $template.find('select.espIsIncomeDeductionSelect').val("false").selectpicker('refresh')
        $template.find('div.espIsIncomeDeductionSelect').hide()
    }
    judgeInvoiceDeductible(false);
}
function ESPDetailDataAppend(data) {
    var count = $('#ESPDetailTable tbody').length;
    var $template = $('#ESPDetailCloneTemplate').clone().attr('id', 'ESPDetail_' + count);
    ...
    var amortizationDateStart = $template.find('.espAmortizationStartDateInput').val() == "" ? "無攤銷起日" : $template.find('.espAmortizationStartDateInput').val();
    var amortizationDateEnd = $template.find('.espAmortizationEndDateInput').val() == "" ? "無攤銷迄日" : $template.find('.espAmortizationEndDateInput').val();
    $template.find('.espAmortizationDateDisable').text(amortizationDateStart + ' - ' + amortizationDateEnd);

    LoadingPaymentCategory($template.find('#espPaymentCategorySelect'), data.PaymentCategory);
    LoadingPaymentMidCategory($template.find('#espPaymentMidCategorySelect'), $template.find('#espPaymentCategorySelect').val(), data.PaymentMidCategory);
    LoadingExpenseAttribute($template.find('#espExpenseAttributeSelect'), $template.find('#espPaymentCategorySelect').val(), $template.find('#espPaymentMidCategorySelect').val(), data.ExpenseAttribute)
    if (data.AccountingItem != null) {
        $template.find('#espAccountingItemDisable').text(data.AccountingItem + ' - ' + data.AccountingItemName)
        $template.find('#espAccountingItemCode').val(data.AccountingItem);
        $template.find('#espAccountingItemName').val(data.AccountingItemName);
    }
    //Table Layer 2

    $template.find('#espOriginalAmountInput').val(data.OriginalAmount);
    $template.find('#espOriginalAmountDisable').text(data.OriginalAmount);
    $template.find('#espOriginalTaxInput').val(data.OriginalTax);
    $template.find('#espOriginalTaxDisable').text(data.OriginalTax);
    $template.find('#espOriginalSaleAmountDisable').text(data.OriginalSaleAmount);
    $template.find('#espOriginalSaleAmountInput').val(data.OriginalSaleAmount);
    $template.find('#espTWDAmountDisable').text(data.TWDAmount);
    $template.find('#espTWDAmountInput').val(data.TWDAmount);
    $template.find('#espTWDTaxDisable').text(data.TWDTax);
    $template.find('#espTWDTaxInput').val(data.TWDTax);
    $template.find('#espTWDSaleAmountDisable').text(data.TWDSaleAmount);
    $template.find('#espTWDSaleAmountInput').val(data.TWDSaleAmount);
    //Table Layer 3
    $template.find('#espIsIncomeDeductionSelect').selectpicker()
    $template.find('#espIsIncomeDeductionSelect').val(data.IsIncomeDeduction.toString());
    $template.find('#espIsIncomeDeductionSelect').selectpicker('refresh')
    if (data.IsIncomeDeduction) {
        $template.find('#espIsIncomeDeductionDisable').text("Y");
    } else {
        $template.find('#espIsIncomeDeductionDisable').text("N");
    }

    LoadingProjectCategory($template.find('#espProjectCategorySelect'), data.ProjectCategory);
    LoadingProject($template.find('#espProjectSelect'), $template.find('#espProjectCategorySelect').val(), data.Project);
    LoadingProjectItem($template.find('#espProjectItemSelect'), $template.find('#espProjectSelect').val(), data.ProjectItem)
    //Setting Value End

    //Table Layer4
    if (data.VoucherMemo == undefined || data.VoucherMemo == "") {
        $template.find('#espVoucherMemoInput').val($('#HeaderDescInput').val());
        $template.find('#espVoucherMemoDisable').text($('#HeaderDescInput').val());
    } else {
        $template.find('#espVoucherMemoInput').val(data.VoucherMemo);
        $template.find('#espVoucherMemoDisable').text(data.VoucherMemo);
    }
...
function LoadingPaymentCategory(target, defaultValue) {
    ...
    if (defaultValue != undefined && defaultValue != null) {
        ...
        $(target).parents('td').find('#espPaymentCategoryDisable').text($(target).find('option:selected').text());
    }
...
function LoadingPaymentMidCategory(target, parentValue, defaultValue) {
    ...
    if (defaultValue != undefined && defaultValue != null) {
        target.val(defaultValue);
        target.selectpicker('refresh');
        $(target).parents('td').find('#espPaymentMidCategoryDisable').text($(target).find('option:selected').text());
    }

    var $expenseAttribute = $(target).parents('tr').find('select.espExpenseAttributeSelect');
    $expenseAttribute.empty();
    $expenseAttribute.selectpicker({ title: '請選擇' }).selectpicker('render');
    DisableDOMObject($expenseAttribute);

    var $espAccountingItemDisable = $(target).parents('tr').find('.espAccountingItemDisable');
    var $AccountingItemInput = $(target).parents('tr').find('input.espAccountingItemInput');
    var $AccountiingItemNameInput = $(target).parents('tr').find('input.espAccountingItemNameInput');
    $espAccountingItemDisable.empty()
    $espAccountingItemDisable.append('<b class="undone-text">系統自動帶入</b>')
...
function LoadingExpenseAttribute(target, firstParentValue, secondParentValue, defaultValue) {
    ...
    if (defaultValue != undefined && defaultValue != null) {
        target.val(defaultValue);
        target.selectpicker('refresh');
        $(target).parents('td').find('#espExpenseAttributeDisable').text($(target).find('option:selected').text());
...
function LoadingAccountingItem(target, firstParentValue, secondParentValue, thirdParentValue) {
    ...
    var $AccountingItemInput = target.parents('tr').find('input.espAccountingItemInput');
    var $AccountiingItemNameInput = $(target).parents('tr').find('input.espAccountingItemNameInput');
    var $espNeedCertificateDisable = target.parents('tbody').find('.espNeedCertificateDisable');
    var $espNeedCertificateInput = target.parents('tbody').find('input.espNeedCertificateInput');
                ...
                if (midItem.PaymentMidCategoryKey == secondParentValue) {
                    $.each(midItem.PaymentAttribute, function (expenseIndex, expenseItem) {
                        if (expenseItem.AttributeValue == thirdParentValue) {
                            $espAccountingItemDisable.empty();
                            $espAccountingItemDisable.append(expenseItem.AccountingValue + ' - ' + expenseItem.AccountingName);
                            ...
                            if (expenseItem.NeedCertificate) {
                                $espNeedCertificateInput.val("Y")
                                $espNeedCertificateDisable.text("Y")
                            } else {
                                $espNeedCertificateInput.val("N")
                                $espNeedCertificateDisable.text("N")
                            }
    ...
    if (firstParentValue == "" || firstParentValue == null || secondParentValue == "" || secondParentValue == null || thirdParentValue == "" || thirdParentValue == null) {
        $espNeedCertificateInput.val("")
        $espNeedCertificateDisable.empty().append('<span class="undone-text">系統自動帶入<span>')
        $espAccountingItemDisable.append(_autoDoneTextIcon)
        $AccountingItemInput.val('');
        $AccountiingItemNameInput.val('')
        return;
    }
...
function LoadingProjectCategory(target, defaultValue) {
    ...
    if (defaultValue != undefined && defaultValue != null) {
        target.val(defaultValue);
        $(target).parents('td').find('#espProjectCategoryDisable').text($(target).find('option:selected').text())
    }
    target.selectpicker('refresh');
}
function LoadingProject(target, parentValue, defaultValue) {
    ...
    if (parentValue != "" && parentValue != null) {
        ...
        if (defaultValue != undefined && defaultValue != null) {
            target.val(defaultValue);
            target.selectpicker('refresh');
            $(target).parents('td').find('#espProjectDisable').text($(target).find('option:selected').text())
        }
    } else {
        var $project = $(target).parents('tr').find('select.espProjectSelect');
        $project.empty();
        $project.selectpicker({ title: '請選擇' }).selectpicker('render');
        DisableDOMObject($project);
    }
    var $projectItem = $(target).parents('tr').find('select.espProjectItemSelect');
    $projectItem.empty();
...
function LoadingProjectItem(target, parentValue, defaultValue) {
    ...
    if (defaultValue != undefined && defaultValue != null) {
        target.val(defaultValue);
        target.selectpicker('refresh');
        $(target).parents('td').find('#espProjectItemDisable').text($(target).find('option:selected').text())
    }
}
function recalculateMoney(e) {
    AccountUnFormat()
    var $this = $('#ESPDetailTable tbody')
    $.each($this, function (index, item) {
        var amount = $(item).find('#espOriginalAmountInput').val();
        var tax = $(item).find('#espOriginalTaxInput').val();
        var exchangeRate = $('#RateInput').val();
        $(item).find('#espOriginalSaleAmountDisable').text(MathRoundExtension(amount - tax, $('#CurrencyPrecisionDisable').text()))
        $(item).find('#espOriginalSaleAmountInput').val(MathRoundExtension(amount - tax, $('#CurrencyPrecisionDisable').text()))
        $(item).find('#espTWDAmountDisable').text(Math.round(amount * exchangeRate))
        $(item).find('#espTWDAmountInput').val(Math.round(amount * exchangeRate))
        $(item).find('#espTWDTaxDisable').text(Math.round(tax * exchangeRate))
        $(item).find('#espTWDTaxInput').val(Math.round(tax * exchangeRate))
        var amountTWD = $(item).find('#espTWDAmountDisable').text();
        var taxTWD = $(item).find('#espTWDTaxDisable').text();
        $(item).find('#espTWDSaleAmountDisable').text(parseInt(amountTWD) - parseInt(taxTWD));
        $(item).find('#espTWDSaleAmountInput').val(parseInt(amountTWD) - parseInt(taxTWD));
    })
    ...
    var totalAmount = 0;
    var totalTax = 0;
    $.each($('#ESPDetailTable tbody'), function (index, item) {
        var $currentItem = $(item).find('tr').eq(2);
        totalAmountTWD += parseInt($currentItem.find('#espTWDAmountDisable').text());
        totalTaxTWD += parseInt($currentItem.find('#espTWDTaxDisable').text());
        totalAmount += parseFloat($currentItem.find('#espOriginalAmountInput').val());
        totalTax += parseFloat($currentItem.find('#espOriginalTaxInput').val());
    });
    ...
    var twd = accounting.unformat($('input[name="TWDAmount"]').val())
    var twdTax = accounting.unformat($('input[name="TWDPayAmount"]').val())
...
function deletedRecalculateMoney(tbodyOfDeleteItem) {
    ...
    var amountMargin = _invoiceCanBeDeductible ? parseFloat(accounting.unformat($(tbodyOfDeleteItem).find('#espOriginalSaleAmountInput').val())) : parseFloat(accounting.unformat($(tbodyOfDeleteItem).find('#espOriginalAmountInput').val()));
    var fixedAmount = [];
    var amortizationDetail = $('#AmortizationDetailTable').find('tbody')
    $.each(amortizationDetail, function (index, item) {
        if (index == amortizationDetail.length - 1) {
...
//==============================================================================================
// 分攤明細層 - 建立/新增(有無資料)/下拉設定/重新計算分攤金額(分攤比率[M]/分攤金額) / 上載
//==============================================================================================
...
function AmortizationTemplateDropSet(template) {
    ...
    if (typeof (Array.prototype.find) == "function") {
        try {
            template.find('#AmortizationDetailCostProfitCenterNameInput').val(_coaCompanyAndDepartmentCollection.find(function (element) { return element.department == template.find('#AmortizationDetailCostProfitCenterSelect').val() }).description)
        } catch (e) {
        }
    }
    ...
    if (typeof (Array.prototype.find) == "function") {
        try {
            template.find('#AmortizationDetailAccountBankNameInput').val(_coaCompany.find(function (element) {
                return element.company == template.find('#AmortizationDetailAccountBankSelect').val()
            }).description)
        } catch (e) {
        }
    }
    ...
    if (!(template.find('#AmortizationDetailExpenseAttributeSelect') == undefined)) {
        $.each(_expenseAttribute, function (index, item) { template.find('#AmortizationDetailExpenseAttributeSelect').append('<option value="' + index + '">' + index + ' ' + item + '</option>') })
        template.find('#AmortizationDetailExpenseAttributeSelect').on('change', function () { template.find('AmortizationDetailExpenseAttributeNameInput').val($(this).find('option:selected').text()) })
    }
    return template;
}
function recalculateAmortizationRatio(e) {
    ...
    var ESPDetail = $('#ESPDetailTable').find('tbody');
    ...
    if (!_invoiceCanBeDeductible) {
        ...
        $.each(ESPDetail, function (index, item) {
            OriginalAmount.push(parseFloat($(item).find('#espOriginalAmountInput').val()));
            TWDAmount.push(parseInt($(item).find('#espTWDAmountDisable').text()));
        ...
        $.each($this.find('tbody'), function (index, item) {
            ...
            for (var i = 0; i < ESPDetail.length; i++) {
        ...
        $.each(ESPDetail, function (index, item) {
            OriginalSaleAmount.push(parseFloat($(item).find('#espOriginalSaleAmountDisable').text()));
            TWDSaleAmount.push(parseInt($(item).find('#espTWDSaleAmountDisable').text()));
...
function recalculateAmortizationRatiobyMoney(e) {
    ...
    //不可扣抵:依照原金額
    if (!_invoiceCanBeDeductible) {
        var totalAmountInESPDetail = 0;
        var totalAmountInAmoritization = 0;
        var totalTWDAmountInESPDetail = 0;
        var totalTWDAmountInAmoritization = 0;

        $.each(ESPDetail, function (index, item) {
            totalAmountInESPDetail += parseFloat($(item).find('#espOriginalAmountInput').val());
            totalTWDAmountInESPDetail += parseInt($(item).find('#espTWDAmountInput').val());
        });
        if (!_moneyNegative && e != undefined && $(e.target).val() > totalAmountInESPDetail) {
        ...
        //計算各明細之分攤比率
        totalAmountInAmoritization = totalAmountInESPDetail;
        totalTWDAmountInAmoritization = totalTWDAmountInESPDetail;
        ...
        $.each(AmortizationDetail, function (index, item) {
            if (index + 1 == AmortizationDetail.length) {
                ...
                $(item).find('#AmortizationDetailRatioInput').val(MathRoundExtension(totalAmountInAmoritization * 100 / totalAmountInESPDetail, 2));
                return;
            }
            ...
            var thisAmortizationRatio = MathRoundExtension(thisValue * 100 / totalAmountInESPDetail, 2);
            var thisTWDValue = Math.round(totalTWDAmountInESPDetail * (thisValue / totalAmountInESPDetail))
        ...
        var totalSaleAmountInESPDetail = 0;
        var totalSaleAmountInAmoritization = 0;
        var totalTWDSaleAmountInESPDetail = 0;
        var totalTWDSaleAmountInAmoritization = 0;

        $.each(ESPDetail, function (index, item) {
            totalSaleAmountInESPDetail += parseFloat($(item).find('#espOriginalSaleAmountDisable').text());
            totalTWDSaleAmountInESPDetail += parseInt($(item).find('#espTWDSaleAmountDisable').text());
        });
        if (!_moneyNegative && e != undefined && $(e.target).val() > totalSaleAmountInESPDetail) {
            alertopen('錯誤：輸入金額大於請款明細層金額總計，請重新輸入');
            $(e.target).val(_previousAmount);
            AccountFormat()
            return;
        ...
        //計算各明細之分攤比率
        totalSaleAmountInAmoritization = totalSaleAmountInESPDetail;
        totalTWDSaleAmountInAmoritization = totalTWDSaleAmountInESPDetail;
        ...
        $.each(AmortizationDetail, function (index, item) {
            if (index + 1 == AmortizationDetail.length) {
                ...
                $(item).find('#AmortizationDetailRatioInput').val(MathRoundExtension(totalSaleAmountInAmoritization * 100 / totalSaleAmountInESPDetail, 2));
                return;
            }
            ...
            var thisValue = $(item).find('#OriginalAmortizationAmountInput').val();
            var thisAmortizationRatio = MathRoundExtension(thisValue * 100 / totalSaleAmountInESPDetail, 2);
            var thisTWDValue = Math.round(totalTWDSaleAmountInESPDetail * (thisValue / totalSaleAmountInESPDetail));

            $(item).find('#OriginalAmortizationAmountDisable').text(thisValue);
            $(item).find('#OriginalAmortizationTWDAmountDisable').text(Math.round(thisTWDValue));
            $(item).find('#OriginalAmortizationTWDAmountInput').val(Math.round(thisTWDValue));
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.ajaxResult && data.ajaxReturn.length > 0) {
                ...
                if (data.ajaxDeduction != undefined && data.ajaxDeduction != null) {
                    if (data.ajaxDeduction) {
                        $('#BTIsDeductionSelect').val('Y').selectpicker('refresh')
                        _invoiceCanBeDeductibleTopAuth = 1;
                    } else {
                        $('#BTIsDeductionSelect').val('N').selectpicker('refresh')
...
function PrepaymentAmountCal(e) {
    ...
    $.each($(ESPDetail).find('tbody'), function (index, item) {
        espDetailTotalAmount += MathRoundExtension(parseFloat($(item).find('#espOriginalAmountInput').val()), parseInt($('#CurrencyPrecisionDisable').text()));
    });
    if (prepaymentReverseTotal > espDetailTotalAmount) {
...
function IncomeTaxCal() {
    ...
    $.each($espDetail, function (index, item) {
        if ($(item).find('#espIsIncomeDeductionSelect').val() != "true") {
            return;
        }
        var espdetailData = {
            lineNumber: 0,
        ...
        espdetailData.lineWTFlag = "Y"
        espdetailData.lineNumber = parseInt($(item).find("#LineNum").val());
        espdetailData.lineAmount = $(item).find('#espTWDAmountInput').val();
        wtTax.detail.push(espdetailData);
    })
    $.ajax({
        async: false,
        type: 'POST',
...
//==============================================================================================
function judgeBusinessTaxGen() {
    var gvdeclaration = $('#VoucherBeauSelect').find('option:selected').attr('gvdeclaration');
    $('#gvdeclaration').val(gvdeclaration)
    var isCreditCardOffice = $('#VoucherBeauSelect').find('option:selected').attr('iscreditcardoffice');
    $('#isCreditCardOffice').val(isCreditCardOffice);
    if ($('#VoucherBeauSelect option:selected').attr('accountcode') == 'C810') {
        $('.BeauCreditCardOpen').show()
    } else {
        $('.BeauCreditCardOpen').hide()
    }
    if (gvdeclaration === "404") {
        _invoiceCanBeDeductibleTopAuth = 0
        $('#BusinessTaxTable').hide();
        $('#undone-BusinessTax').show();
        ...
        $businessTaxDetail.find('div[id$="Disable"]').text("")
        $businessTaxDetail.find('input[id$="Input"]').val("")
        $businessTaxDetail.find('select[id$="Select"]').val("")
        return;
    } else {
...
function BusinessTax() {
    ...
    $('#BTCertificateDateDisable').text($('#CertificateDateInput').val())
    $('#BTCertificateDateInput').val($('#CertificateDateInput').val())
    ...
    $('#BTIsDeductionSelect').selectpicker()
...
function BusinessTaxDataLoading(data) {
    ...
    if (data.BusinessNum != null) {
        ...
        $('#BusinessNumInput').val("");
        $('#BusinessNumDisable').text("無資料")
    }
...
function CertificateKindChange() {
    ...
    switch ($('#CertificateKindSelect option:selected').val()) {
        ...
        default:
            ...
            $('.taxfield').text(0);
            $.each(espDetail, function (index, item) {
                $(item).find('#espOriginalSaleAmountDisable').text($('#espOriginalAmountInput').val())
                $(item).find('#espOriginalSaleAmountInput').val($('#espOriginalAmountInput').val())
                $(item).find('#espTWDSaleAmountDisable').text($('#espTWDAmountInput').val())
                $(item).find('#espTWDSaleAmountInput').val($('#espTWDAmountInput').val())
            })
            SwitchHideDisplay([$('#CertificateNumDisable')], [$('#CertificateNumInput')])
            RemoveRequired($('#CertificateNumTitle'));
            RemoveRequired($('#EstimateVoucherDateTitle'));
...
function judgeInvoiceDeductible(e) {
    ...
    if (_invoiceCanBeDeductibleTopAuth == 1) {
        ...
        //分攤明細層可不可扣抵判定
        var $amortizationDetail = $('#AmortizationDetailTable tbody');
        $.each($amortizationDetail, function (index, item) {
    ...
    if (invoiceDeductibleOri != $('#systemJudgeDeductibleInput').val()) {
        ...
        var totalTax = 0;
        $.each($('#ESPDetailTable tbody'), function (index, item) {
            totalTax += parseFloat($(item).find('#espOriginalTaxInput').val());
        })
...
function removeAllData() {
    var espTable = $('#ESPDetailTable')
    $.each(espTable.find('tbody'), function (index, item) {
        if ($(item).find('#LineNum').val() != 0) {
            $('#InformationZone').append('<input type="hidden" name="ESPDetail.Index" value="' + $(item).find('#LineNum').val() + '">')
            $('#InformationZone').append('<input type="text" hidden name="ESPDetail[' + $(item).find('#LineNum').val() + '].LineNum" value="' + $(item).find('#LineNum').val() + '">')
            $('#InformationZone').append('<input type="text" hidden name="ESPDetail[' + $(item).find('#LineNum').val() + '].IsDelete" value="true">')
        }
    })
    espTable.remove();
