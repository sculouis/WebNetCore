...
function bindings_create(model, template, schema) {
    ...
    //日曆控件取得值bind處理
    self.on('dp.change', '.datetimepicker1', function (e) {
        var path = e.currentTarget.firstElementChild.id
        var model = self.data('model')
        value = e.currentTarget.firstElementChild.value
        bindings_setvalue(model, path, value, schema)
    })

    //隱藏欄位更改值bind處理  
    $('input[type=hidden]').bind("change", function (e) {
    ...
    self.on('change', 'input[name]', function (e) {
        bindings_internal_change.call(this, e, self, self.data('model'), schema);
    });

    self.on('change', 'textarea[name],select[name]', function (e) {
        bindings_internal_change.call(this, e, self, self.data('model'), schema);
    });
...
function bindings_destroy() {
    ...
    self.find('input[name],textarea[name],select[name]').unbind('change');
...
function bindings_validate(schema) {
    ...
    bindings_reflection(model, function (path, value, key) {
        ...
        if (typeof (r) === 'undefined' || r === null || r)
            return;
        error.push({ path: path, value: value, element: self.find('input[name="' + path + '"],textarea[name="' + path + '"],select[name="' + path + '"]') });
    });
...
function bindings_set(path, value, schema) {
    ...
    $.bindings.watch.call($('input[name="' + path + '"],textarea[name="' + path + '"],select[name="' + path + '"]'), r, path, value, model, schema);
...
//輸入參數說明obj:資料model,path:控制項id,value:控制項內容
